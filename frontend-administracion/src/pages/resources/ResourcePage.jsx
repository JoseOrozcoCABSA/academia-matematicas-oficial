...
import { BookOpen, Image, Layers3, MoveRight, Pencil, Plus, RefreshCw, Search, Shapes, Trash2 } from 'lucide-react';
  ...
  return (
    <div className="page-content">
      ...
      {isAreaCatalog && <section className="area-organizer">
        <section className="lesson-catalog-help"><Shapes /><div><h2>Selecciona una categoría para administrarla</h2><p>Filtra por nivel y grado. Usa <strong>Ver lecciones</strong> para revisar el contenido y moverlo a otra categoría sin perder secciones, actividades ni archivos.</p></div></section>
        <section className="admin-panel">
          <div className="panel-heading"><div><h2>Categorías</h2><p>{areaCategories.length} de {total} registros</p></div><div className="table-tools"><label><Search /><input placeholder="Buscar categoría…" value={search} onChange={(event) => setSearch(event.target.value)} /></label><button className="admin-secondary icon-only" onClick={() => load()}><RefreshCw /></button></div></div>
          {error && <div className="alert error">{error}</div>}
          <div className="lesson-filter-panel"><div className="lesson-filter-heading"><div><strong>Encuentra una categoría rápidamente</strong><small>Combina nivel educativo, grado y búsqueda libre.</small></div><span>{areaCategories.length} resultados</span></div><div className="lesson-filter-grid area-filter-grid">
            <label><span>Nivel educativo</span><select value={areaPath.level} onChange={(event) => chooseLevel(event.target.value)}><option value="">Todos los niveles</option>{areaLevels.map((level) => <option value={level} key={level}>{level.charAt(0).toUpperCase() + level.slice(1)}</option>)}</select></label>
            <label><span>Grado</span><select value={areaPath.grade} disabled={!areaPath.level} onChange={(event) => chooseGrade(event.target.value)}><option value="">Todos los grados</option>{areaPath.level && gradesForLevel(areaPath.level).map((grade) => <option value={grade} key={grade}>{gradeLabel(grade)}</option>)}</select></label>
            <button className="admin-secondary lesson-filter-clear" disabled={!areaPath.level && !areaPath.grade && !search} onClick={() => { chooseLevel(''); setSearch(''); }}>Limpiar filtros</button>
          </div><p className="lesson-filter-tip"><Search /> La búsqueda revisa nombre, descripción, nivel, slug y color de la categoría.</p></div>
          <div className="lesson-admin-grid area-admin-grid">{areaCategories.map((area) => { const lessons = areaLessons.filter((lesson) => String(lesson.category_id) === String(area.id) && (!areaPath.grade || lessonGradeCode(lesson) === areaPath.grade)); return <article key={area.id} className={`lesson-admin-card ${String(areaPath.category) === String(area.id) ? 'selected' : ''}`}><div className="lesson-admin-card-head"><span>{area.icon || <Shapes />}</span><i className={area.active ? 'published' : ''}>{area.active ? 'Visible' : 'Oculta'}</i></div><div className="lesson-relation-path"><span>{area.education_level || 'Sin nivel'}</span><b>›</b><span>{areaPath.grade ? gradeLabel(areaPath.grade) : 'Todos los grados'}</span><b>›</b><strong>Categoría</strong></div><h3>{area.name}</h3><p>{area.description || 'Sin descripción. Puedes agregarla desde los datos generales.'}</p><div className="lesson-admin-meta"><span>Slug: {area.slug}</span><span>{lessons.length} lecciones</span><span>Orden: {area.sort_order ?? 0}</span></div><div className="lesson-admin-actions"><button className="admin-primary" onClick={() => chooseCategory(area)}><BookOpen /> Ver lecciones</button><button className="admin-secondary" onClick={() => setEditor({ record: area })}><Pencil /> Datos</button></div></article>; })}</div>
          {loading && <div className="table-state">Cargando información…</div>}{!loading && !areaCategories.length && <div className="table-state">No se encontraron categorías.</div>}
        </section>
        {areaPath.category && <section className="admin-panel area-lessons-step"><div className="panel-heading"><div><h2>Lecciones de {selectedCategory?.name}</h2><p>{selectedAreaLessons.length} lecciones {areaPath.grade ? `en ${gradeLabel(areaPath.grade)}` : 'en todos los grados'}. Selecciona una para editarla.</p></div><button className="admin-secondary" onClick={() => setAreaPath((current) => ({ ...current, category: '' }))}>Cerrar</button></div><div className="lesson-relation-path area-current-path"><span>{selectedCategory?.education_level}</span><b>›</b><span>{areaPath.grade ? gradeLabel(areaPath.grade) : 'Todos los grados'}</span><b>›</b><strong>{selectedCategory?.name}</strong><b>›</b><span>Lecciones</span></div>{selectedAreaLessons.length ? <div className="area-lesson-list">{selectedAreaLessons.map((lesson) => <article key={lesson.id}><button type="button" className="area-lesson-select" onClick={() => setLessonWorkspace(lesson)}><BookOpen /><span><strong>{lesson.title}</strong><small>{gradeLabel(lessonGradeCode(lesson)) || 'Grado no identificado'} · {lesson.slug}</small></span></button><div className="area-lesson-controls"><div className="lesson-admin-actions"><button type="button" className="admin-primary" onClick={() => setLessonWorkspace(lesson)}><Layers3 /> Editar contenido</button><button type="button" className="admin-secondary" onClick={() => setEditor({ record: lesson })}><Pencil /> Datos</button></div><label><span><MoveRight /> Mover a otra categoría</span><select value={lesson.category_id || ''} disabled={String(movingLessonId) === String(lesson.id)} onChange={(event) => void relocateLesson(lesson, event.target.value)}>{rows.filter((area) => area.education_level === selectedCategory?.education_level).map((area) => <option value={area.id} key={area.id}>{area.name}</option>)}</select></label></div></article>)}</div> : <div className="table-state">No hay lecciones en esta categoría y grado.</div>}</section>}
      </section>}
