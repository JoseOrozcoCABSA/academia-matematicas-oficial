  ...
  return (
    <div className="page-content">
      ...
      {isAreaCatalog && <section className="area-organizer">
        ...
        {areaPath.category && <section className="admin-panel area-lessons-step"><div className="panel-heading"><div><h2>Lecciones de {selectedCategory?.name}</h2><p>{selectedAreaLessons.length} lecciones {areaPath.grade ? `en ${gradeLabel(areaPath.grade)}` : 'en todos los grados'}</p></div><button className="admin-secondary" onClick={() => setAreaPath((current) => ({ ...current, category: '' }))}>Cerrar</button></div><div className="lesson-relation-path area-current-path"><span>{selectedCategory?.education_level}</span><b>›</b><span>{areaPath.grade ? gradeLabel(areaPath.grade) : 'Todos los grados'}</span><b>›</b><strong>{selectedCategory?.name}</strong><b>›</b><span>Lecciones</span></div>{selectedAreaLessons.length ? <div className="area-lesson-list">{selectedAreaLessons.map((lesson) => <article key={lesson.id}><div><BookOpen /><span><strong>{lesson.title}</strong><small>{gradeLabel(lessonGradeCode(lesson)) || 'Grado no identificado'} · {lesson.slug}</small></span></div><label><span><MoveRight /> Mover a otra categoría</span><select value={lesson.category_id || ''} disabled={String(movingLessonId) === String(lesson.id)} onChange={(event) => void relocateLesson(lesson, event.target.value)}>{rows.filter((area) => area.education_level === selectedCategory?.education_level).map((area) => <option value={area.id} key={area.id}>{area.name}</option>)}</select></label></article>)}</div> : <div className="table-state">No hay lecciones en esta categoría y grado.</div>}</section>}
      </section>}
