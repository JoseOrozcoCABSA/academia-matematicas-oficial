...
export default function SectionEditorPage({ section, lesson, sections, media, sectionTypes, optionLabels, Preview, onBack, onSaved }) {
  ...
  const save = async (event) => {
    ...
    try {
      ...
      await onSaved({
        ...
        ai_exercise_count: Math.max(3, Math.min(20, Number(draft.ai_exercise_count || 10))),
      }, file);
    } catch (reason) {
  ...
  return <main className="page-content section-editor-page">
    ...
    <form onSubmit={save} className="section-editor-form">
      {exerciseError && <div className="alert error section-save-error"><strong>No fue posible guardar.</strong> {exerciseError}</div>}
      ...
      <section className="section-type-options">
        ...
        {isAssessment && <div className="section-ai-options"><strong>¿Cómo deseas crear este examen?</strong><div className="exam-mode-selector"><button type="button" className={!draft.ai_exercises_enabled ? 'active' : ''} onClick={() => setDraft({ ...draft, ai_exercises_enabled: false })}><b>Creado por mí</b><small>Escribe y edita cada pregunta y respuesta.</small></button><button type="button" className={draft.ai_exercises_enabled ? 'active' : ''} onClick={() => setDraft({ ...draft, ai_exercises_enabled: true })}><b>Generado con IA</b><small>Usa el proveedor configurado centralmente.</small></button></div>{draft.ai_exercises_enabled ? <><div className="model-config-grid"><label>Tipo de ejercicios<select value={draft.ai_exercise_type || 'generic'} onChange={(event) => setDraft({ ...draft, ai_exercise_type: event.target.value })}><option value="generic">Problemas generales</option><option value="addition">Sumas</option><option value="vertical_addition">Sumas verticales</option><option value="fractions">Fracciones</option><option value="mixed">Evaluación mixta</option><option value="multiple_choice">Opción múltiple</option></select></label><label>Cantidad<input type="number" min="3" max="20" value={draft.ai_exercise_count || 10} onChange={(event) => setDraft({ ...draft, ai_exercise_count: event.target.value })} /></label><label>Dificultad<select value={draft.ai_difficulty || 'Básica'} onChange={(event) => setDraft({ ...draft, ai_difficulty: event.target.value })}><option>Básica</option><option>Intermedia</option><option>Avanzada</option></select></label></div><label>Instrucciones adicionales para la IA<textarea rows="4" placeholder="Ejemplo: usa situaciones cotidianas para primaria y retroalimentación sencilla." value={draft.ai_prompt || ''} onChange={(event) => setDraft({ ...draft, ai_prompt: event.target.value })} /></label></> : <p className="manual-exam-notice">Agrega abajo preguntas de opción múltiple o respuesta directa. Podrás modificarlas antes de publicar.</p>}</div>}
      </section>
