...
const TAB_TEMPLATES = [
  { type: 'presentation', title: 'Lección', icon: '📘', description: 'Apartado principal con explicaciones y recursos del tema.' },
...
export default function LessonStructurePage({ lesson, onBack }) {
  ...
  const missingDefaultTabs = defaultTabs.filter((template) => !roots.some((root) => root.section_type === template.type));
  const createDefaultTabs = async () => {
    if (!missingDefaultTabs.length) return;
    setCreatingDefaults(true); setError('');
    try {
      await Promise.all(missingDefaultTabs.map((template, index) => gatewayApi.create('learning', 'aprendizaje_secciones_leccion', {
        ...
        sort_order: roots.length + index + 1, published: true, ai_exercises_enabled: false,
      })));
      await load();
    } catch (reason) { setError(reason.message); }
  ...
  return <div className="page-content lesson-structure-page">
    ...
    <div className="lesson-structure-toolbar"><div><h2>Apartados principales de la lección</h2><p>Por defecto son Lección, Actividad y Evaluación. Puedes editarlos o añadir otro apartado padre cuando sea necesario.</p></div><div className="lesson-toolbar-actions">{missingDefaultTabs.length > 0 && <button type="button" className="admin-secondary" disabled={creatingDefaults} onClick={createDefaultTabs}><Plus /> {creatingDefaults ? 'Creando…' : 'Completar 3 apartados predeterminados'}</button>}<button type="button" className="admin-primary" onClick={() => setCreationTarget({ mode: 'tab', parentId: null })}><Plus /> Añadir apartado padre</button></div></div>
