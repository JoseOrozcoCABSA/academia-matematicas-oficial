...
const TAB_TEMPLATES = [
  { type: 'presentation', title: 'Lección', icon: '📘', description: 'Pestaña principal con la explicación del tema.' },
  { type: 'activity', title: 'Actividad', icon: '✏️', description: 'Pestaña para práctica, ejercicios y trabajo guiado.' },
  { type: 'evaluation', title: 'Evaluación', icon: '✅', description: 'Pestaña de evaluación corta y calificación.' },
  { type: 'exam', title: 'Examen', icon: '📝', description: 'Pestaña para una evaluación formal o final.' },
];
const CONTENT_TYPES = [
...
export default function LessonStructurePage({ lesson, onBack }) {
  ...
  const SectionNode = ({ section, depth = 0 }) => <div className="lesson-tree-branch">
    <article className={`lesson-tree-node depth-${Math.min(depth, 3)}${String(selectedId) === String(section.id) ? ' selected' : ''}`} onClick={() => setEditingSection(section)}>
      ...
      <div className="lesson-tree-actions" onClick={(event) => event.stopPropagation()}><button type="button" onClick={() => setEditingSection(section)}><Pencil /> Editar</button>{!depth && <button type="button" onClick={() => setCreationTarget({ mode: 'content', parentId: section.id, parentTitle: section.title })}><Plus /> Agregar contenido</button>}<button type="button" className="danger" onClick={() => removeSection(section)}><Trash2 /></button></div>
    </article>
  ...
  return <div className="page-content lesson-structure-page">
    ...
    <div className="lesson-structure-toolbar"><div><h2>Pestañas visibles para el estudiante</h2><p>Cada pestaña padre aparece en la parte superior de la lección. Dentro puedes agregar cualquiera de los 10 tipos de contenido.</p></div><button type="button" className="admin-primary" onClick={() => setCreationTarget({ mode: 'tab', parentId: null })}><Plus /> Crear nueva pestaña</button></div>
    {creationTarget && <section className="lesson-type-picker"><header><div><small>{creationTarget.mode === 'tab' ? 'PASO 1 · NUEVA PESTAÑA PADRE' : `PASO 2 · CONTENIDO DENTRO DE “${creationTarget.parentTitle}”`}</small><h2>{creationTarget.mode === 'tab' ? '¿Qué pestaña verá el estudiante?' : '¿Qué deseas agregar?'}</h2><p>{creationTarget.mode === 'tab' ? 'Después podrás colocar lecturas, videos, documentos y otros recursos dentro.' : 'Selecciona uno de los 10 tipos disponibles.'}</p></div><button type="button" className="admin-secondary" onClick={() => setCreationTarget(null)}>Cancelar</button></header><div className={creationTarget.mode === 'tab' ? 'lesson-type-grid tab-types' : 'lesson-type-grid'}>{(creationTarget.mode === 'tab' ? TAB_TEMPLATES.map((item) => [item.type, item.icon, item.title, item.description]) : CONTENT_TYPES).map(([type, icon, title, description]) => <button type="button" key={`${type}-${title}`} onClick={() => chooseCreationType(type, title)}><b>{icon}</b><strong>{title}</strong><small>{description}</small><span>Seleccionar →</span></button>)}</div></section>}
