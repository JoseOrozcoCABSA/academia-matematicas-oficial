...
export class ResourcesRepository {
  ...
  private cleanData(
    ...
    if (definition.tableName === 'practica_juegos' && clean.config_json !== undefined) {
      let config: Record<string, unknown>;
      try {
        config = (typeof clean.config_json === 'string'
          ? JSON.parse(clean.config_json)
          : clean.config_json) as Record<string, unknown>;
      } catch {
        throw new AppError('La configuración del juego no es JSON válido', 400, 'INVALID_GAME_CONFIG');
      }
      const challenges = config?.challenges;
      if (!Array.isArray(challenges) || challenges.length === 0) {
        throw new AppError('El juego necesita al menos una ecuación', 400, 'INVALID_GAME_CONFIG');
      }
      challenges.forEach((item, index) => {
        const challenge = item as Record<string, unknown>;
        const a = Number(challenge.a);
        const b = Number(challenge.b);
        const c = Number(challenge.c);
        if (![a, b, c].every(Number.isInteger) || a <= 0 || b < 0 || c <= b || (c - b) % a !== 0) {
          throw new AppError(
            `La ecuación ${index + 1} debe producir una solución entera positiva`,
            400,
            'INVALID_GAME_EQUATION',
          );
        }
      });
      clean.config_json = JSON.stringify(config);
    }
    return clean;
  }
