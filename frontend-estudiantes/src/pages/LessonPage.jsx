...
export function ExerciseBlock({ section, lesson, user, onComplete }) {
  ...
  const load = async () => {
    ...
    try {
      const data = await aiApi.generate({ user_id: user.id, lesson_id: lesson.id, section_id: section.id, topic: lesson.title, count: 10, type: section.section_type });
  ...
  if (!items.length) return <div className="exercise-launch"><BrainIcon /><div><h3>{sectionLabel(section.section_type)} interactiva</h3><p>Genera 10 ejercicios desde el modelo central configurado y recibe tu calificación.</p></div><button onClick={load} disabled={loading}>{loading ? 'Generando…' : 'Comenzar'}</button>{error && <small>{error}</small>}</div>;
  return <div className="embedded-exercises"><header><div><span>{sectionLabel(section.section_type)}</span><h3>Resuelve los ejercicios</h3>{modelInfo && <small>Motor: {modelInfo.name}{modelInfo.fallback ? ' · respaldo local' : ''}</small>}</div><button onClick={load}>Generar otros</button></header>{modelInfo?.warning && <div className="exercise-provider-warning">{modelInfo.warning}</div>}<div className="exercise-grid">{items.map((item, index) => <label className={result ? (normalize(answers[item.id]) === normalize(item.answer) ? 'correct' : 'wrong') : ''} key={item.id}><span>{index + 1}</span><div><b>{item.question}</b>{item.operands?.length ? <strong className="operation">{item.operands.join(` ${item.symbol || '+'} `)} =</strong> : null}<input disabled={Boolean(result)} value={answers[item.id] ?? ''} onChange={(event) => setAnswers({ ...answers, [item.id]: event.target.value })} placeholder="Respuesta" />{result && normalize(answers[item.id]) !== normalize(item.answer) && <small>Respuesta correcta: {item.answer}</small>}</div></label>)}</div>{!result ? <button className="primary-action" onClick={submit}>Finalizar {section.section_type === 'evaluation' ? 'evaluación' : 'actividad'}</button> : <div className={result.score >= 70 ? 'result-banner pass' : 'result-banner fail'}><Trophy /><div><strong>{result.score}%</strong><span>{result.correct} de {items.length} respuestas correctas</span></div></div>}</div>;
}
